...
import { createMainWindow, findGuid, getWinDetails } from './windows'
...
ipcMain.on('update-window-details', async (event) => {
    async function getDetails() {
        ...
        for (const win of windows) {
            windowDetails.push(await getWinDetails(win))
        }
...
ipcMain.on('app-config-changed', () => {
    // Will update the config file.
    // Saves the update
    // Returns the updpated appConfig.
})
